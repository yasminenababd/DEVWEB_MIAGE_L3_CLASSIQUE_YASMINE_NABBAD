	...
	<table>
        ...
        <tbody>
    	   ...
    	   <?php
                ...
                $reponce = $bdd->query('SELECT * FROM médicaments WHERE Stock=0');
                // Requpete donnant accès aux données de la table médicaments ou le stock est null
   
                if($reponce->rowCount() == 0) {
                    //Si la requête n'a pas été exécutée, un message d'erreur s'affiche
                    
                    ?> 
                    <p class="errortext"><?php echo "Aucun médicaments n'est en rupture de stock."; 
